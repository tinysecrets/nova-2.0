...
class VoiceInterface:
  def __init__(self,enabled:bool=True):
    self.enabled=enabled; self.tts_engine=None
    if pyttsx3 and self.enabled:
      try: self.tts_engine=pyttsx3.init()
      except Exception: self.tts_engine=None
  def speak(self,text:str)->None:
    print(f"[NOVA Voice Output]: {text}")
    if self.tts_engine:
      try: self.tts_engine.say(text); self.tts_engine.runAndWait()
      except Exception as e: print(f"[Voice TTS Warning]: {e}",file=sys.stderr)
  def listen(self,audio_file_path:str=None)->str:
    if audio_file_path and whisper:
      try: return whisper.load_model("base").transcribe(audio_file_path).get("text","")
      except Exception as e: print(f"[Voice STT Error]: {e}",file=sys.stderr)
    return ""