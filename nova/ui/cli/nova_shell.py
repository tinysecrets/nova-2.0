...
def run_shell():
  mind=NovaMind(); voice=VoiceInterface()
  print("==========================================")
  print("      Welcome to NOVA 2.0 Shell           ")
  print("==========================================")
  print("Type 'help' for available commands or 'exit' to quit.\n")
  while True:
    try: user_input=input("NOVA> ").strip()
    except (KeyboardInterrupt,EOFError):
      print("\nExiting NOVA Shell."); break
    if not user_input: continue
    if user_input.lower() in ["exit","quit"]: print("Goodbye!"); break
    if user_input.lower()=="help":
      print("Available commands:",", ".join(COMMANDS)); continue
    if user_input.lower()=="profile":
      print("User Profile:",mind.user_profile.to_dict()); continue
    matches=fuzzy_match(user_input)
    if matches and user_input not in COMMANDS: print(f"Did you mean: {', '.join(matches)}?")
