...
@app.get("/api/tasks")
def get_tasks(): return [t.to_dict() for t in mind.get_all_tasks()]
@app.post("/api/tasks")
def create_task(task_data:TaskCreate): return mind.add_task(title=task_data.title,category=task_data.category).to_dict()
...
def delete_task(task_id:str):
  if not mind.delete_task(task_id): raise HTTPException(status_code=404,detail="Task not found")
  return {"status":"deleted","id":task_id}
@app.post("/api/query")
def process_query(req:QueryRequest): return mind.process_query(req.query)
@app.websocket("/ws/monitor")
async def websocket_endpoint(websocket:WebSocket):
  await manager.connect(websocket)
  try:
    await websocket.send_json({"status":"connected","agent":"NOVA 2.0","autonomy":mind.user_profile.autonomy})
    while True: await websocket.send_json(mind.process_query(await websocket.receive_text()))
