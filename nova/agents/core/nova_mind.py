...
class TaskItem:
  def __init__(self, title: str, completed: bool = False, task_id: str = None,
...
class NovaMind:
  def __init__(self, storage_path: str = None):
    ...
    self.storage_path = storage_path or os.path.expanduser("~/.nova_todo_tasks.json")
    ...
    if os.path.exists(self.storage_path):
      try:
        with open(self.storage_path, "r", encoding="utf-8") as f:
          data = json.load(f)
        self.tasks = [TaskItem(title=i.get("title",""), completed=i.get("completed",False),
                               task_id=i.get("id"), category=i.get("category","general"),
                               priority=i.get("priority","Medium"), due_date=i.get("due_date"))
                      for i in data]
      except Exception:
        self.tasks = []
  ...
  def add_task(self, title: str, category: str = "general",
               priority: str = "Medium", due_date: Optional[str] = None) -> TaskItem:
    task = TaskItem(title=title, category=category, priority=priority, due_date=due_date)
  ...
  def process_query(self, user_input: str) -> Dict[str, Any]:
    ...
    if user_input_lower.startswith("add todo ") or user_input_lower.startswith("add task "):
      ...
      response_text = f"Added task: '{task.title}' [{task.priority} Priority] (ID: {task.id[:8]})"
      action = "add_task"
    elif user_input_lower in ["list todo", "list tasks", "show todos", "show tasks"]:
      tasks_str = "\n".join(
          f"[{'X' if t.completed else ' '}] [{t.priority}] {t.id[:8]}: {t.title}"
          for t in self.tasks)
    ...
    self.conversation_history.add_entry(role="assistant", content=response_text,
                                        metadata={"action": action})
    return {"response": response_text, "action": action,
            "autonomy": self.user_profile.autonomy, "tasks_count": len(self.tasks)}